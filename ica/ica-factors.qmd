...
> The bars aren't in logical order
...
> We'd have to manually check each level of support and then type every state out in order by hand
...
```{r}
grade_distribution |>
  mutate(grade = fct_rev(fct_infreq(grade, n))) |>
  ggplot(aes(x = grade, y = n)) +
    geom_col()
```
