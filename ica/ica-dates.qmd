...
\ It might eliminate observations with data that we'd like to look at
\
\
\
...
```{r}
# Gets the year from the date today
year(today)
```

```{r}
# What do these lines produce / what's their difference?
# Gets the month value as both a numeric value and a string
month(today)
...
# Number of weeks into the year
week(today)
```

```{r}
# What do these lines produce / what's their difference?
# The day numbered from the beginning of the month and the beginning of the year
mday(today)
...
```{r}
# What do these lines produce / what's their difference?
# Represents the day of the week as a numeric value and a string
wday(today)
...
# Tells us whether today is before, after, or on the specified date
...
```{r}
daily_births |>
  ggplot(aes(x = date, y = births, color = weekday)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE)
```
...
Birthdays |>
  ...
  mutate(month_births = sum(births)) |>
  ggplot(aes(x = month, y = month_births)) +
  geom_point(alpha = 0.5)
```
...
```{r}
births_per_week <- Birthdays |>
  mutate(week = week(date)) |>
  group_by(state, week) |>
  filter(year == 1988, week(date) < 53) |>
  summarize(births_per_week = sum(births)) 
...
Birthdays |>
  ...
  group_by(state) |>
  summarize(births_in_period = sum(births)) |>
  arrange(desc(births_in_period)) |>
  head(5)
```
