...
# Units of observation = penguin groups of sex and species
head(penguin_avg)
```
...
sleep_wide |>
  pivot_longer(cols = -Subject, names_to = "day", values_to = "time") |>
  head(3)
```
...
sleep_long |>
  ...
  head()
```
> It's not clear what the columns are keeping track of
...
billboard |>
  ...
  select(track, artist, ratings_increase_wk1to2)
```
...
```{r}
most_appearances_by_year <- daily |>
  group_by(raw_guest_list, year) |>
  summarize(appearances = n()) |>
  mutate(total_appearances = sum(appearances)) |>
  arrange(year) |>
  pivot_wider(names_from = year, values_from = appearances, values_fill = 0) |>
